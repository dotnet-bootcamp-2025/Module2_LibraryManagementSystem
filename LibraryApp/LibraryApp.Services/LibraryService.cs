...
namespace LibraryApp.Services
{
    public sealed class LibraryService : ILibraryService
    {
        ...
        public IReadOnlyList<LibraryItem> listAll() => _items;

        public IEnumerable<LibraryItem> search(string term)
        {
            if (string.IsNullOrWhiteSpace(term)) return Enumerable.Empty<LibraryItem>();
            return _items.Where(i => i.Title.Contains(term, StringComparison.OrdinalIgnoreCase));
        }
        public void Seed()
        {
